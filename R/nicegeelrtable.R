...
#' Lisa's GEE Logistic Regression Table Function - fit must be geeglm type model 
#'
#' @param df Dataframe. 
#' @param fit Logistic Regression geeglm (geepack) model object.
...
nicegeelrtable <- function(df, 
    ...
    if (overallp == TRUE){
        names(tbl) <- c("Variable", "Odds Ratio", "95% CI", "Wald p-value", "Chisq p-value")
    }
    if (overallp == FALSE){
